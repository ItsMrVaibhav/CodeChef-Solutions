    ...
    while (T--)
    {
        ...
        for (int i = 0; i < N; i++)
            for (int j = 0; j < M; j++)
            {
                if (i > 0)
                {
                    if (board[i][j].first == board[i - 1][j].first)
                        merge(board[i - 1][j].second, board[i][j].second);
                    else
                        edges.emplace_back(make_pair(board[i - 1][j].second, board[i][j].second));
                }
                if (j > 0)
                {
                    if (board[i][j].first == board[i][j - 1].first)
                        merge(board[i][j - 1].second, board[i][j].second);
                    else
                        edges.emplace_back(make_pair(board[i][j].second, board[i][j - 1].second));
                }
            }
